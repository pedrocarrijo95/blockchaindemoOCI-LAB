...
//-----------------   PASSO 1. CRIAR STRUCTS ------------------------------//

		//gera etiquetas e distribui (gerador->familias)
		...
		//ensaca os itens e envia para cooperativa (familias->cooperativas)
		...
		//envio da cooperativa para processamento
		type Processar struct{
			CodigoLote string `json:"CodigoLote"` //(as 2 cooperativas precisam corresponder nesse codigo) para que tenhamos o histórico de quem participou do lote final da IndustriaX.
			DataRegistro string `json:"DataRegistro"` //timestamp
		...
		//processamento enviado para IndustriaX - industrializacão
		...
		//IndustriaX recebe e gera a transacão para finalizar em unidades de produto
		type Finalizado struct{
			CodigoLoteIndustriaX string `json:"CodigoLoteIndustriaX"`
			DataRegistro string `json:"DataRegistro"` //timestamp
...
//-----------------   PASSO 2. CRIAR FUNCÕES ------------------------------//
		...
		func (s *SmartContract) Invoke(APIstub shim.ChaincodeStubInterface) sc.Response {
			...
			} else if function == "addIndustriaX"{
				return s.addIndustriaX(APIstub)
		...
		func (s *SmartContract) addEtiqueta(APIstub shim.ChaincodeStubInterface) sc.Response {
			...
			if(string(strjson) != "[]"){
				return shim.Error("Essa etiqueta ja existe, tente utilizar outro codigo.")
			}
		...
		func (s *SmartContract) addIndustriaX(APIstub shim.ChaincodeStubInterface) sc.Response {
			_, args := APIstub.GetFunctionAndParameters()

			if len(args) != 5 {
				return shim.Error("Incorrect number of arguments for event. Expecting 5")
			};
			CodigoLoteIndustriaX := args[0]
			if CodigoLoteIndustriaX == "" {
				return shim.Error("Invalid value for parameter CodigoLoteIndustriaX.")
			}
			DataRegistro := args[1]
			if DataRegistro == "" {
				return shim.Error("Invalid value for parameter DataRegistro.")
			...
			errorMsg := registerIndustriaX(APIstub,CodigoLoteIndustriaX,DataRegistro,Quantidade,TipoProduto,CodigoLoteProcessamento)
			if errorMsg != "" {
				erromsg := "Erro no segundo err: "+errorMsg;
				return shim.Error(erromsg)
			}
		...
		func registerIndustriaX(APIstub shim.ChaincodeStubInterface, codigoLoteIndustriaX string, dataRegistro string, quantidade string, tipoProduto string, codigoLoteProcessamento string) (string) {
			//time1 := time.Now()
			//time1 := "tempo"
			newPosition := Finalizado{CodigoLoteIndustriaX: codigoLoteIndustriaX, DataRegistro: dataRegistro, Quantidade: quantidade, TipoProduto: tipoProduto, CodigoLoteProcessamento: codigoLoteProcessamento }
			positionEncoded, _ := json.Marshal(newPosition)
			err := APIstub.PutState(APIstub.GetTxID(), positionEncoded)
			if err != nil {
				return fmt.Sprintf("Failed to register" , "-")
